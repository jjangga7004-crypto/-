...
| 도구 | 설명 |
|------|------|
| `start_iq_test` | 새 테스트를 시작합니다. `num_questions`(3~전체)로 문항 수 지정, `name`(선택). 세션 ID와 첫 문제를 반환합니다. |
| `answer_question` | 현재 문제에 답을 제출합니다. `session_id`, `choice`("A"~"D" 또는 0~3). 정답 피드백 + 다음 문제(또는 최종 결과)를 반환합니다. |
| `test_status` | 진행 상황과 현재 문제를 다시 보여줍니다. |
| `abort_test` | 테스트를 중단하고 지금까지 답한 문항으로 채점합니다. |

문제는 5개 분야(수열 `sequence`, 논리 `logic`, 언어유추 `verbal`,
수리 `math`, 공간 `spatial`)에서 난이도를 가중치로 출제·채점합니다.
...
1. `start_iq_test(num_questions=10)` → 세션 ID + 1번 문제
2. `answer_question(session_id, choice="C")` → 정답 여부 + 2번 문제
3. … 마지막 문항까지 반복
4. 마지막 답을 내면 자동으로 **추정 IQ / 분류 / 백분위 / 분야별 정답**이 나옵니다.
